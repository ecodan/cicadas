...
- **Spec-Native Context Contract** — The five core initiative templates (`prd.md`, `ux.md`, `tech-design.md`, `approach.md`, `tasks.md`) now share machine-readable front matter with `summary`, `modules`, `depends_on`, and `index` fields. Semantic routing lives with the spec it describes rather than in a separate context manifest.
- **Reset Boundaries over Memory Assumptions** — Cicadas now defines Branch Reset, Phase Reset, and Partition Reset rules in `SKILL.md`. These rules tell agents to prefer approved file-backed state and indexed sections first, and only ask the host to clear or compact context opportunistically when supported.
...
- **Compact Context First**: At branch start and other reset boundaries, agents should reload `canon/summary.md`, the active specs' front matter, and only the indexed sections needed for the current task before opening full documents.
