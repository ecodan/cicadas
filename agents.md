...
- **[src/cicadas/](src/cicadas/)**: Contains the core logic of the orchestrator, including the repo-local common CLI at `scripts/cicadas.py`, its command registry, lifecycle scripts (kickoff, branch, status, create_lifecycle, open_pr, review, emit_event, get_events, validate_skill, skill_publish, unarchive, tutorial), an interactive 7-step tutorial (`tutorial.py`) invoked on first `init` or any time via `cicadas tutorial`, a hint subsystem in `utils.py` (`hints_enabled`, `print_hint`) that prints next-step guidance after every lifecycle command, adaptive canon/bootstrap and targeted reconcile logic in `scripts/scan_repo.py`, `scripts/synthesize.py`, and `scripts/utils.py`, the optional code-graph toolchain (`graph_build.py`, `graph_query.py`, `graph_observe.py`, staged SQLite storage, Java/JS extractors, and the Java semantic harness), emergence instruction modules (the **standard start flow** for initiative/tweak/bug entrypoints plus a deprecated legacy skill path in `emergence/start-flow.md`, deprecated `skill-create.md` / `skill-edit.md` authoring docs retained for compatibility, plus **Building on AI** — gate and eval status in start flow, optional eval spec for initiatives, eval/benchmark reminder for tweaks/bugs), and spec/canon templates including seeded slice packs for large/mega repos. The five core initiative templates now share compact front matter and section indexes so agents can restart from approved file-backed context.
