...
- `scripts/`: The repo-local CLI entrypoint `cicadas.py`, its command registry, and the underlying deterministic tools for project lifecycle operations (kickoff, branch, status, create_lifecycle, open_pr, review, tokens, emit_event, get_events, validate_skill, skill_publish, unarchive, tutorial, etc.). `tutorial.py` implements the interactive 7-step tutorial invoked by `cicadas tutorial` or `cicadas init --tutorial`. All lifecycle scripts (kickoff, branch, archive, update_index, open_pr, init) now print a 💬 next-step hint via the `print_hint()` helper in `utils.py`; `status.py` infers the hint from registry state. The optional code-graph subsystem also lives here: `graph.py` dispatches graph commands, `graph_build.py` and `graph_store.py` manage staged SQLite-backed graph artifacts under `.cicadas/graph/`, `graph_query.py` handles routed lookups, `graph_observe.py` renders progress/watch output, `graph_usage.py` records local usage/value metadata with end-to-end timing, and `graph_eval.py` runs local scenario-based graph quality checks. Python extraction uses AST, Java graph extraction uses a structural baseline plus resumable semantic enrichment batches, and JavaScript/TypeScript plus Rust use fallback structural extraction with optional Tree-sitter capability reporting when local packages and grammars are available. `utils.py` includes `emit()` — a shared non-fatal event emitter (lazy-imports `emit_event`) used by kickoff, branch, archive, and open_pr.
- `emergence/`: Instruction modules for the drafting phase. Includes `start-flow.md` — the mandatory sequence for current initiative/tweak/bug entrypoints, plus a deprecated legacy skill path retained for compatibility. `skill-create.md` and `skill-edit.md` remain in the repo as deprecated legacy Agent Skill authoring modules. When work involves LLMs, `eval-spec.md` guides creation of an eval spec (initiatives only); Approach asks eval placement. Tweaks/bugs get an optional eval/benchmark reminder. Choices stored in `emergence-config.json`. Cicadas does not run evals. `clarify.md` now refreshes approved front matter fields rather than older `steps_completed` metadata.
...
- `graph/`: Optional local graph artifacts. `codegraph.sqlite`, `metadata.json`, `area-plan.json`, `progress.json`, and `progress-log.jsonl` appear after `python src/cicadas/scripts/cicadas.py graph build`; `spool/` contains streamed node/edge JSONL, `tools/` contains extractor-side artifacts such as Java semantic batch manifests/logs, and `usage.jsonl` captures local graph usage/value events, result summaries, overlap-ready fields, and end-to-end timings.
...
When starting an initiative, tweak, or bug, the agent runs the **standard start flow** first (see `emergence/start-flow.md`): name, draft folder, **LLMs and Evals?** (yes/no; if yes, eval status), then requirements source and pace for initiatives, then PR preference. A deprecated legacy skill path still exists in `start-flow.md` for compatibility, including publish-destination handling. For work involving LLMs, the agent may later offer an eval spec (initiatives) or an eval/benchmark reminder (tweaks/bugs). Vague ideas are refined into structured drafts in `.cicadas/drafts/{initiative}/`.
...
- **Optional Code Graph**: When repo owners build `.cicadas/graph/`, agents can use graph queries to route from symptoms, failing tests, files, descriptions, and symbols without loading large swaths of the repo. `graph search`, `graph route`, `graph neighbors`, `graph callers`, `graph callees`, `graph tests`, `graph signature-impact`, `graph tail`, `graph watch`, `graph usage`, and `graph eval` now share the same local graph workspace. Builds stage SQLite rows progressively, emit UTC progress logs with ETA, and keep area planning inspectable through `area-plan.json`. `graph eval --repo path --scenario-file scenarios.jsonl --output report.json` runs synthetic or private local scenario suites and writes iterative metrics for top-N hit rate, rank, coverage, latency, and failures. Tree-sitter is optional; analyzer metadata reports whether fallback structural or Tree-sitter extraction was used. When the graph is absent, Cicadas falls back to canon-first routing with no workflow penalty.
...
`tests/test_graph.py` covers the optional code graph CLI, including build/status behavior, routed queries, local observability output such as `usage.jsonl` entries and `graph usage` summaries, Java semantic batching, optional Rust/Tree-sitter fallback metadata, and staged graph persistence behavior. `tests/test_graph_eval.py` covers scenario parsing, synthetic fixture generation, eval report metrics, and CLI execution. `tests/test_scan_repo.py` covers repo inventory, source-aware scale classification, slice seeding, and scan exclusion logic — including that markdown-heavy SDD/spec/docs trees remain visible as context without independently forcing large/mega repo mode.
