...
- `graph/`: Optional local graph artifacts. `codegraph.sqlite` and `metadata.json` appear only after `python src/cicadas/scripts/cicadas.py graph build`; `usage.jsonl` captures local graph usage/value events and end-to-end timings.
...
- **Optional Code Graph**: When repo owners build `.cicadas/graph/`, agents can use graph queries to route from symptoms, failing tests, files, and symbols without loading large swaths of the repo. When the graph is absent, Cicadas falls back to canon-first routing with no workflow penalty.
...
`tests/test_graph.py` covers the optional code graph CLI, including build/status behavior, routed queries, and local observability output such as `usage.jsonl` entries and `graph usage` summaries.
